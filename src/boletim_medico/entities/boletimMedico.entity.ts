...
export class BoletimMedico {
  ...
  @ApiProperty({
    description: 'Descrição do boletim médico',
    example: 'Paciente estável, sem febre.',
  })
  @Column({ length: 1000 })
  descricao: string;
  ...
  @ApiProperty({
    description: 'Pressão arterial do paciente',
    example: '120/80',
  })
  ...
  @ApiProperty({
    ...
    example: 'Paciente apresentou melhora após administração de medicação.',
  })
  @Column({ length: 1000, nullable: true })
  observacoes: string;

  @ApiProperty({ description: 'Internação relacionada ao boletim médico' })
  @ManyToOne(() => Internacao, (internacao) => internacao.boletinsMedicos, {
    onDelete: 'CASCADE',
  })
